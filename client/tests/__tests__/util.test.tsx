...
describe('util', () => {
  describe('url', () => {
    it('returns url with full path', () => {
      expect(url('xxx')).toBe('http://petclinic-dev.us-west-2.elasticbeanstalk.com//xxx');
    });
  });
  ...
  describe('submitForm', () => {
    ...
    it('submits all data', () => {
      ...
      return submitForm('POST', '/some-enzyme', { name: 'Test' }, (status, response) => {
        ...
        expect(fetchMock.mock.calls[0][0]).toBe('http://petclinic-dev.us-west-2.elasticbeanstalk.com///some-enzyme');
